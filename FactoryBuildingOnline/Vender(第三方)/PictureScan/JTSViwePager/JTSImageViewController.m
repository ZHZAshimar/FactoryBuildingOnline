...
- (void)imageLongPressed:(UILongPressGestureRecognizer *)sender {
    ...
    UIAlertAction *copyImage = [UIAlertAction actionWithTitle:@"保存图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"tupian");
        [self saveImageToPhotos:self.imageView.image];
    }];
//    UIAlertAction *copyURL = [UIAlertAction actionWithTitle:@"拷贝图片链接" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        NSLog(@"链接");
//    }];
//    [alertView addAction:copyURL];
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        NSLog(@"取消");
    }];
    [alertView addAction:copyImage];
...

{
    ...
    __block JTSImageViewController *weakSelf = self;
...
//    [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(removeAlertView:) userInfo:alertView repeats:NO];
}
