...
#import "UIImageView+CustomImageView.h" // 添加 UIimageview 的类目
...
- (void)initData
{
    ...
    {
        // 修改图片加载方式 调用UIImageView 类目
        [bigImage setImageWithURLStr:browseItem.bigImageUrl];
        //        [bigImage sd_setImageWithURL:[NSURL URLWithString:browseItem.bigImageUrl]];
        
        CGRect verticalRect = [bigImage.image jdy_getBigImageRectSizeWithScreenWidth:JDY_SCREEN_WIDTH screenHeight:JDY_SCREEN_HEIGHT];
        NSValue *verticalValue = [NSValue valueWithCGRect:verticalRect];
        [_verticalBigRectArray addObject:verticalValue];
        
        CGRect horizontalRect = [bigImage.image jdy_getBigImageRectSizeWithScreenWidth:JDY_SCREEN_HEIGHT screenHeight:JDY_SCREEN_WIDTH];
