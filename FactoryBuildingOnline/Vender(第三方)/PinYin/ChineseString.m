...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr forKey:(NSString*)key
{
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:stringArr forKey:key];
    ...
    for (NSString* object in tempArray)
    {
        ...
        NSString *pinyin = chineseString.pinYin.length ? [chineseString.pinYin substringToIndex:1] :@"";
        //不同
...
#pragma mark - 返回联系人
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr forKey:(NSString*)key
{
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:stringArr forKey:key];
...
//
///////////////////
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr forKey:(NSString*)key
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        chineseString.string=[NSString stringWithString:[stringArr objectAtIndex:i][key]];
        chineseString.name = stringArr[i][@"strRealName"];
        chineseString.userID = stringArr[i][@"nFriendID"];
    
        chineseString.avator = stringArr[i][@"strAvatar"];
...
//        chineseString.string =[ChineseString RemoveSpecialCharacter:chineseString.string];
        ...
        //判断首字符是否为字母
        NSString *regex = @"[A-Za-z]+"; NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
        NSString *initialStr = [chineseString.string length]?[chineseString.string substringToIndex:1]:chineseString.string;
