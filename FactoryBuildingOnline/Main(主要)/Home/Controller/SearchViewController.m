...
- (void)viewDidLoad {
    ...
    NSArray *tmpArr = [SearchFile readFileArray];          // 从沙盒中读取文件
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar {
    ...
    if ([self.writeArray containsObject:searchBar.text]) {
        ...
        if ([SearchFile writeSearchFileArray:self.writeArray]) {    // 写入文件
            
            self.searchCollectionView.historyArray = [SearchFile readFileArray];
...
- (SearchVCCollectionView *)searchCollectionView {
    ...
    if (!_searchCollectionView) {
        ...
        _searchCollectionView.deleteFile = ^(BOOL flag) {
            ...
            weakSelf.searchCollectionView.historyArray = [SearchFile readFileArray];    // 更新数组的内容
        };
