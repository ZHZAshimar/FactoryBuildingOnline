...
#import "ECFactoryBusinessInputView.h"
...
@interface ECFactoryBusinessInputView ()<UITableViewDelegate, UITableViewDataSource,UITextFieldDelegate>
{
...
@implementation ECFactoryBusinessInputView
...
- (void)swichButtonDidTouch:(UISwitch*)sender {
    ...
    isOn = sender.on;
    // 刷新第八个section
    NSIndexSet *indexSet = [[NSIndexSet alloc] initWithIndex:9];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    switch (section) {
        ...
        case 1:case 4:case 5: case 7:case 8:case 10:
...
- (UITableViewCell *)swichCellTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        ...
        // 取值
        NSMutableArray *array = self.selectedArray[9];
        if (array.count > 1) {
        ...
        cell.inputBlock = ^(NSString *inputText){
            ...
            [weakSelf.selectedArray replaceObjectAtIndex:9 withObject:array];
        };
        return cell;
    } else {
...
- (UITableViewCell *)text_textButtonTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 若有值，进行赋值
    NSArray *array = self.selectedArray[6];
    if (array.count > 0) {
...
- (UITableViewCell *)inputNumTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        ...
        cell.inputBlock = ^(NSString *text) {
            NSArray *array = [NSArray arrayWithObject:text];
            [weakSelf.selectedArray replaceObjectAtIndex:7 withObject:array];
        };
    }
...
- (UITableViewCell *)pullDownTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        ...
        case 5:
        {
            NSArray *array = @[@"经纪费用",@"经纪费用承担方",@"经济费用支付时间"];
            
            NSArray *mArr = self.selectedArray[5];
            
            NSDictionary *dic = mArr[indexPath.row];
            if (dic.count > 0) {
                cell.contentLabel.text = mArr[indexPath.row][@"name"];
            } else {
                cell.contentLabel.text = array[indexPath.row];
            }
        ...
        case 8:
        {
            NSDictionary *dic = self.selectedArray[8][indexPath.row];
...
#pragma mark - 选择身份
        case 0:
        {
            ...
            self.pickView.dataSource = @[@"请选择您的身份",@"房东",@"经纪人", @"租户"];
...
#pragma mark - 选择租赁用途
        ...
        {
            self.pickView.hidden = NO;
            self.pickView.dataSource = @[@"请选择租赁用途",@"仓库",@"生产",@"办公"];
            self.pickView.selectStrBlock = ^(NSString *selectStr,NSInteger index){
                if (index < 1) {
                    return ;
                }
...
#pragma mark - 租期开始时间
        case 5:
        {
            NSMutableArray *mArr = [NSMutableArray arrayWithArray:self.selectedArray[5]];
            self.pickView.hidden = NO;
            if (indexPath.row == 0) {
                self.pickView.dataSource = @[@"请选择经纪费用",@"金额10%",@"30天租金",@"45天租金",@"60天租金"];
                self.pickView.selectStrBlock = ^(NSString *selectStr,NSInteger index){
                    if (index < 1) {
                        return ;
                    }
                ...
                self.pickView.dataSource = @[@"请选择经纪费用承担方",@"出租人（房东）",@"承租人(租户)",@"指定第三方"];
                self.pickView.selectStrBlock = ^(NSString *selectStr,NSInteger index){
                    if (index < 1) {
                        return ;
                    }
                    ...
                    [mArr replaceObjectAtIndex:1 withObject:@{@"name":selectStr,@"index":@(index-1)}] ;   // 将数据插入已选数组中
                    [weakSelf.selectedArray replaceObjectAtIndex:5 withObject:mArr];
                    [weakSelf.myTabelView reloadData];
                };
            } else {
                self.pickView.dataSource = @[@"请选择经纪费用支付时间",@"租户签订合同",@" 租户入住"];
                self.pickView.selectStrBlock = ^(NSString *selectStr,NSInteger index){
                    if (index < 1) {
                        return ;
                    }
                    ...
                    [mArr replaceObjectAtIndex:2 withObject:@{@"name":selectStr,@"index":@(index-1)}] ;   // 将数据插入已选数组中
                    [weakSelf.selectedArray replaceObjectAtIndex:5 withObject:mArr];
                    [weakSelf.myTabelView reloadData];
                };
            }
        ...
        case 6:
        {
            ...
            self.dateView.timeBlock = ^(NSDate *date){
                ...
                NSMutableArray *mArr = [NSMutableArray arrayWithArray:weakSelf.selectedArray[6]];
                [mArr replaceObjectAtIndex:indexPath.row withObject:cell.contentLabel.text];
                [weakSelf.selectedArray replaceObjectAtIndex:6 withObject:mArr];
                NSLog(@"%@",weakSelf.selectedArray);
            };
        }
            break;
...
#pragma mark - 押金支付时间
        case 9:
        {
            ...
            if (indexPath.row == 2) {
                NSMutableArray *mArr = [NSMutableArray arrayWithArray:self.selectedArray[9]];
                self.dateView.hidden = NO;
                self.dateView.timeBlock = ^(NSDate *date){
                    ...
                    [weakSelf.selectedArray replaceObjectAtIndex:9 withObject:mArr];
                    [weakSelf.myTabelView reloadData];
                };
            }
        }
